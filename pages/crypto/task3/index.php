...
$k=0;
$flag=1;
for($i=0;$i<5;$i++)for($j=$k;$j<5+$k;$j++){
	if($flag && chr(ord('z')-$i*5-$j)=='i'){
		$flag=0;
		$polybius['i']=$i.($j-1);
		$k=1;
	} else $polybius[''.chr(ord('z')-$i*5-$j)]=$i.($j-$k);
}
//$polybius['a']='44';
//print_r($polybius);
	...
	<body>
		...
		<div id="content">
			...
			<div id="question">
				...
				Derp, the hacker found out Derpina's encrypted password to be "<b>0020022142310410430122112313</b>" (quotes for clarity) and he guesses that Derpina uses polybius encryption.But Derpina is known for doing things in reverse, so help Derp to decrypt her password.      </p>
			</div>
			<div align="center">
